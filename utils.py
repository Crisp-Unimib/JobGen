...
def sample_skills(code_4d, df_occupations_and_skills, df_skills_count_per_occupation, global_skill_frequency):

    code_3d = int(code_4d[:-3])  # 4 digit code
    ...
    try:
        # il numero di skill da pescare deve essere al 4 digit (la somma dei 5)
        lambda_poisson = int(
            occupation_df['skill_to_job_ratio_3d'].iloc[0] / 2)
    except:
        lambda_poisson = 10
    ...
    skill_list_for_this_3d['code_3d'] = skill_list_for_this_3d['code_4d'].str[:-3]
    skill_list_for_this_3d = skill_list_for_this_3d[skill_list_for_this_3d['code_3d'] == str(
        code_3d)]
    ...
    if not global_skill_frequency_filtered.empty:
        ...
        # compute the distance between 3 digit code and code_4d
        # skill_list_for_this_3d['distance'] = skill_list_for_this_3d['code_4d'].apply(lambda x: abs(int(x.replace('.','')) - int(code_3d)*100))
        skill_list_for_this_3d['distance'] = skill_list_for_this_3d['code_4d'].str[3:].str.replace(
            '.', '')  # in teoria basta fare cosi

        # merge skill_list_for_this_3d with global_skill_frequency_filtered in preferedLabel_skill and escoskill_level_3
        global_skill_frequency_filtered = global_skill_frequency_filtered.merge(skill_list_for_this_3d[[
                                                                                'preferredLabel_skill', 'distance']], left_on='escoskill_level_3', right_on='preferredLabel_skill', how='inner')
        # drop duplicates in escoskill_level_3
        global_skill_frequency_filtered.drop_duplicates(
            subset='escoskill_level_3', inplace=True)
        ...
        # NON E' GLOBAL MA DEL QUARTO DIGIT FILTRATO AL TERZO + PESATURA IN BASE AL DIGIT
        global_skill_frequency_filtered['probability_rescaled'] = global_skill_frequency_filtered['cnt'] / prob_sum

        # normalize inverse of distance
        global_skill_frequency_filtered['distance'] = 1 / (
            global_skill_frequency_filtered['distance'].replace(0, np.inf).astype('float'))
        ...
        a = 0.9
        global_skill_frequency_filtered['final_importance'] = global_skill_frequency_filtered['probability_rescaled'] * (
            1 - a) + a * (1 / global_skill_frequency_filtered['distance'].astype('float'))

        # Ensure not to sample more skills than available
        n_skills_to_pick = min(n_skills_to_pick, len(
            global_skill_frequency_filtered))
        print("Skills available for sampling:",
              len(global_skill_frequency_filtered))
        print(
            f"λ/2 (Poisson mean): {lambda_poisson}, value drawn: {n_skills_to_pick}")
    ...
    # if not global_skill_frequency_filtered.empty else selected_skills['preferredLabel_skill']
    return selected_skills['escoskill_level_3']
...
def rescale_counts(df):
    ...
    def scale_log_cnt(log_cnt, target_min, target_max, desired_mean):
        ...
        # [0,1] * 28 + 2 --> [2, 30]
        range_scaled = scaled_cnt * (target_max - target_min) + target_min
        ...
        corrected_scaled_cnt = np.clip(
            corrected_scaled_cnt, target_min, target_max)
...
def pick_seed_offline(code, oja_example_df):
    ...
    oja_example_df['job_ad'] = oja_example_df['title'] + \
        '. ' + oja_example_df['description']
...
if __name__ == "__main__":
    ...
    df_counts_4d = pd.read_csv("E:/uni/thesis/data/counts_4d.csv")
    rescaled_df = rescale_counts(df_counts_4d)
    for code_4d_value in df_counts_4d.code_4d.sample(5):
        # 1324.3
        print("Rescaled count for", code_4d_value,
              ":", get_rescaled_count(code_4d_value))
